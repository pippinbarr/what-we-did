...
class PrayState extends WWDState
{
	...
	private function checkNote(justPressed:Bool,justReleased:Bool, theNote:FlxSound):Void
	{
		...
		if (justPressed && (!theNote.playing || theNote.volume < 0.2)) { anyPlayed = true; theNote.volume = 0.4; theNote.play(true); }
		else if (justReleased && theNote.playing) { theNote.fadeOut(0.2); }
		if (theNote.volume == 0) { theNote.stop(); theNote.volume = 1; }
