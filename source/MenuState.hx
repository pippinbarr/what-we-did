...
class MenuState extends FlxState
{
	...
	override public function create():Void
	{
		...
		if (loadNewOptions)
		{
			...
			for (i in 0...3) currentOptions.push(allOptions[i]);
		...
		option0.x = 100; option0.y = 200;
		if (currentStrikes[0]) option0.animation.frameIndex = STRIKE_FRAME;

		option1 = getOptionSprite(currentOptions[1]);
		option1.x = 125; option1.y = 250;
		if (currentStrikes[1]) option1.animation.frameIndex = STRIKE_FRAME;

		option2 = getOptionSprite(currentOptions[2]);
		option2.x = 150; option2.y = 300;
		if (currentStrikes[2]) option2.animation.frameIndex = STRIKE_FRAME;

		option3 = drive;
		option3.x = 175; option3.y = 350;
		if (currentStrikes[3]) option3.animation.frameIndex = STRIKE_FRAME;
	...
	private function optionTextFadedIn():Void
	{
		...
		new FlxTimer(2,optionTextFadeOut);
		// new FlxTimer(0.1,optionTextFadeOut);
	}
	...
	{
		FlxG.camera.stopFX();
		new FlxTimer(4,carmenFadeOut);
		// new FlxTimer(0.1,carmenFadeOut);
	}
