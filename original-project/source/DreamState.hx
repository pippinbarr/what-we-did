...
class DreamState extends WWDState
{
	...
	override public function create():Void
	{
		...
		limitTimer = new FlxTimer(1*60,dreamLimit);
	}
	...
	private function handleDreamShift():Void
	{
		...
		else
		{
			...
			if (Math.random() > 0.5) dreamSprites[fadeInIndex].scale.x *= -1;
			if (Math.random() > 0.5) dreamSprites[fadeInIndex].scale.y *= -1;
		}
	}
