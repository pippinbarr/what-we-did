...
class DriveState extends WWDState
{
	...
	override public function create():Void
	{
		...
		else
		{
			...
			{
				if (i/history.length < 0.1) continue;
		...
		if (carX != -1) car.x = carX;
		if (carY != -1) car.y = carY;
		if (carAngle != -1) car.angle = carAngle;
		...
		new FlxTimer(MARKER_DELAY,drawMarker);
	...
	override public function update():Void
	{
		...
		if (car.x + car.width/2 < 0) car.x += FlxG.width;
		else if (car.x + car.width/2 > FlxG.width) car.x -= FlxG.width;
		if (car.y + car.height/2 < 0) car.y += FlxG.height;
		else if (car.y + car.height/2 > FlxG.height) car.y -= FlxG.height;
